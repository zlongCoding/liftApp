...
import 'package:myapp/pages/bibi/bibiDetail.dart';
import 'package:myapp/utils/utils.dart';
import 'package:myapp/widget/icon.dart';
import 'package:myapp/widget/imageViewer.dart';

class BibiBox extends StatelessWidget {
  final BibiItemModule data;
  final bool isDetail;

  BibiBox({this.data, this.isDetail = false});

  @override
  Widget build(BuildContext context) {
    List<Widget> renderContent() {
      List<Widget> list = [];
      list.add(Text('${data.pubtime} 发布',
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (BuildContext context) =>
                          ImageViwer(src: '$base_url${data.imgs[0].src}')));
            },
            child: Container(
                height: 230.0,
          ...
          child: Row(
            ...
            children: <Widget>[
              Row(children: <Widget>[
                LiftIcon(
                    code: 0xe8ca,
                    color: data.likenum >= 100 ? Colors.red : Colors.grey),
                SizedBox(width: 4.0),
                Text('${data.likenum}')
              ]),
              Text('|', style: TextStyle(color: Color(0xffeeeeee))),
              Row(children: <Widget>[
                LiftIcon(code: 0xe635),
                SizedBox(width: 4.0),
                Text('${data.comments}')
              ])
            ],
