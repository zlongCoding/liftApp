...
import 'package:myapp/dao/article.dart';
import 'package:myapp/dao/channel.dart';
import 'package:myapp/module/article.dart';
import 'package:myapp/module/channel.dart';
import 'package:myapp/pages/layout/article/articleBox.dart';
import 'package:myapp/pages/layout/search.dart';
import 'package:myapp/utils/utils.dart';
import 'package:myapp/widget/Empty.dart';
import 'package:myapp/widget/LoadMore.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Widget _selectItem(String title, int index, int total,
      [bool showLine = true]) {
    List<Widget> _list = [];
    ...
    if (_curIndex == index) {
      _list.add(LiftIcon(code: 0xe665, size: 28.0, color: Colors.black));
    } else {
  ...
  List _selectItemList() {
    ...
    for (var i = 0; i < channels.length; i++) {
      _channels.add(_selectItem(channels[i].name, channels[i].id,
          channels[i].articlecount, i != channels.length - 1));
    }
              ...
              Row(
                  ...
                  children: <Widget>[
                    ...
                    GestureDetector(
                      ...
                      child: LiftIcon(
                          code: 0xe63c, color: Colors.black, size: 24.0),
                    )
                  ]),
              Center(
                  child: Container(
  ...
  Widget build(BuildContext context) {
    return PageWrap(
        header: _header(),
        body: LoadingContainer(isLoading: loading, child: renderMain()));
  }
}
