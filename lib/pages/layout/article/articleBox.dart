...
import 'package:myapp/module/article.dart';
import 'package:myapp/pages/layout/article/articleDetail.dart';
import 'package:myapp/utils/utils.dart';
import 'package:myapp/widget/Avatar.dart';
import 'package:myapp/widget/icon.dart';

class ArticleBoxs extends StatelessWidget {
  final ArticleItemModule article;
  final void Function(String title, int index) channelClick;

  ArticleBoxs({this.article, this.channelClick});
    ...
    content.add(Padding(
        ...
        child: Column(
            ...
            children: <Widget>[
              ...
              Row(children: <Widget>[
                LiftIcon(code: 0xe638, size: 16.0),
                SizedBox(width: 8.0),
                Text(article.pubtime,
                    style: TextStyle(fontSize: 14.0, color: commonBaseColor))
              ]),
              Container(
                        ...
                        ChannelWrap(
                            ...
                            onClick: () {
                              if (channelClick != null) {
                                channelClick(
                                    article.channel.name, article.channel.id);
                              }
                            }),
                        Row(children: <Widget>[
...
class ChannelWrap extends StatelessWidget {
  ...
  ChannelWrap({this.name, this.cover, this.onClick});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        child:
          ...
          Avatar(url: '$base_url$cover', size: 30.0, border: false),
          SizedBox(width: 4.0),
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Center(
          ...
          Text(text, style: TextStyle(fontSize: 12.0, color: Color(0xff4c4c4e)))
        ])));
  }
}
...
class CutLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Text('|', style: TextStyle(color: Color(0xffeeeeee))));
  }
}
...
class DataIcon extends StatelessWidget {
  ...
  DataIcon({this.code, this.count = 0});

  @override
  Widget build(BuildContext context) {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
      LiftIcon(code: code, color: Color(0xffaaaaaa)),
      SizedBox(width: 4.0),
